...
def predict_two_classes(examples: List[FactExample], fact_checker):
    ...
    for i, example in enumerate(tqdm(examples)):
        ...
        if gold_label == 1 and pred_label == 0:
            false_pos.append(example)
        if gold_label == 0 and pred_label == 1:
            false_neg.append(example)

    print_eval_stats(confusion_mat, gold_label_indexer)
    print('False Positives')
    false_pos_facts = []
    for ex in false_pos:
        print("Fact: '"+ex.fact+"', Label: "+ex.label)
        false_pos_facts.append(ex.fact)
    print('False Negatives')
    false_neg_facts = []
    for ex in false_neg:
        print("Fact: '"+ex.fact+"', Label: "+ex.label)
        false_neg_facts.append(ex.fact)

    # with open('false_pos.json', 'w') as file:
    #     json.dump(false_pos_facts, file)
    # with open('false_neg.json', 'w') as file:
    #     json.dump(false_neg_facts, file)
